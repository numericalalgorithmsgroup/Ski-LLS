...
int main(int argc, char **argv){
    ...
    if ( (Araw->m) < (Araw->n) ){
        A_sparse = cs_dl_transpose(Araw,true);
        cs_dl_free(Araw);
    }
    else{
        A_sparse = Araw;
    }
    std::cout << "number of rows is: " << A_sparse->m << std::endl;
    std::cout << "number of columns is: " << A_sparse->n << std::endl;
    std::cout << "number of non-zeros is: " << A_sparse->nzmax << std::endl;
    cholmod_l_finish(cc);

    Mat_d A = toDense(A_sparse);
    Mat_d Acopy = A.copy(); // This is only necessary as now CPQR modifies A
